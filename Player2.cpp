...
void Player2::_ready() {
    ...
    UtilityFunctions::print("Player2 encontrado: ", player2 != nullptr);
...
/*void Player2::handle_right_click() {
    if (selected_tank && map_cpp_node && ground_tile_map) {
        ...
        if (path.size() > 0) {
            ...
            for (int i = path.size() - 1; i >= 0; --i) {
                Vector2 target_position = path[i];
                UtilityFunctions::print("Tanque movido a celda: ", target_position);
                ...
                current_position = selected_tank->get_global_position();
            }
